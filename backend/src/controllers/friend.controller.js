...
export const sendFriendRequest = async (req, res) => {
    try {
        ...
        if (alreadyFriends) {
            return res.status(400).json({ message: "Cả hai đã là bạn bè" });
        }
        if (existingRequest) {
...
export const acceptFriendRequest = async (req, res) => {
    try {
        ...
        console.error("Lỗi khi chấp nhận cầu kết bạn", error);
        return res.status(500).json({ message: "Lỗi hệ thống" })
    }
};
export const declineFriendRequest = async (req, res) => {
    try {
        ...
        await FriendRequest.findByIdAndDelete(requestId);
        return res.status(200).json({
            message: "Đã từ chối lời mời", data: {
                requestId,
                status: "declined"
            }
        })
...
export const getAllFriends = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: "Lấy thành công", friends });
    } catch (error) {
...
export const getFriendsRequest = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Lấy yêu cầu kết bạn thành công", sent, received })
    } catch (error) {
        console.error("Lỗi khi lấy danh sách bạn bè", error);
        return res.status(500).json({ message: "Lỗi hệ thống" })
    }
